...
export const options = [
	...
	{
		argname: '-d, --diff',
		description: 'Compute the difference between two URLs. Only applicable when using --file and two URLs are provided.',
		defaults: false,
	},
	{
...
function outputResults( opt, results ) {
	...
	if ( opt.diff ) {
		if ( tableData.length !== 2 ) {
			log(
				formats.error( `The --diff option was ignored because you provided ${ results.length } URL(s), but it requires two URLs to compare.` )
			);
		} else {
